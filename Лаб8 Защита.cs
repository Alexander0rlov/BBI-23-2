...
class Task15 : Task
{
    ...
    public override void ParseText(string text)
    {
        Regex regex = new Regex(@"(?<![,.])-?[0-9]+(?![0-9,.])"); //совпадения с простыми числами с 1 или несколькими вхождениями
        ...
        if (matches.Count > 0)
        {
            foreach (Match match in matches)
            {
                ...
                Console.WriteLine(Convert.ToDouble(match.Value));
            }
        }

        Regex rx = new Regex(@"-?\d+,\d+");
        MatchCollection matchess = rx.Matches(text);
        ...
        Regex years = new Regex(@"\b\d+[,.](?![\d.,])");
        MatchCollection matchesss = years.Matches(text);


        if (matchesss.Count > 0)
        {
            foreach (Match match in matchesss)
            {
                string m = match.ToString();
                string n = Regex.Replace(m, "[.,]", "");
                Console.WriteLine(n);
...
class Program
{
    static void Main()
    {
        ...
        Task15 task15 = new Task15(text2);
